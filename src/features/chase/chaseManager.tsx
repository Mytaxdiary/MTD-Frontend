...
import {
  mockChaseClients as chaseClients,
  mockChaseTemplates as defaultTemplates,
} from '@/mocks/chase/chaseData'
...
export default function ChaseManager({
  ...
  const saveEdit = () => {
    setTemplates((ts) =>
      ts.map((t) => (t.id === editingTemplate ? { ...t, body: editBody, subject: editSubject } : t))
    )
                  ...
                  {filteredOverdue.map((c, i) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: '14px 16px',
                        borderBottom:
                          i < filteredOverdue.length - 1 ? `1px solid ${B.borderLight}` : 'none',
                        background: sent.has(c.id)
                          ? B.greenBg
                          : selected.has(c.id)
                            ? '#F0F9FF'
                            : 'transparent',
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <span>
                            Due: <b style={{ color: B.redText }}>{c.deadline}</b> ({c.daysOverdue}d
                            overdue)
                          </span>
                          <span>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setClientChannels({ ...clientChannels, [c.id]: e.target.value })
                            }
                            style={{
                              fontSize: 10,
                              padding: '2px 6px',
                  ...
                  {filteredUpcoming.map((c, i) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: '14px 16px',
                        borderBottom:
                          i < filteredUpcoming.length - 1 ? `1px solid ${B.borderLight}` : 'none',
                        background: sent.has(c.id)
                          ? B.greenBg
                          : selected.has(c.id)
                            ? '#F0F9FF'
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <span>
                            Due: <b style={{ color: B.text }}>{c.deadline}</b> (
                            {Math.abs(c.daysOverdue)}d left)
                          </span>
                          <span>
                            Chased: <b style={{ color: B.text }}>{c.chaseCount}x</b>
                          </span>
                    ...
                    >
                      <div
                        style={{
                          fontSize: 12,
                          fontWeight: 600,
                          color: selectedTemplate === t.id ? B.blueText : B.text,
                        }}
                      >
                        {t.name}
                      </div>
                      ...
                      >
                        {t.type === 'bookkeeping' ? 'BK' : t.type === 'data-request' ? 'DR' : 'GEN'}
                      </span>
                    </div>
                  </div>
                ))}
            ...
            >
              ...
              >
                ...
                {!editingTemplate && currentTemplate && (
                  <button
                    onClick={() => startEdit(currentTemplate)}
                    style={{
                      fontSize: 11,
                      fontWeight: 500,
                      padding: '4px 10px',

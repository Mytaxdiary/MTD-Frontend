...
const QDot = ({ status }) => {
  const colors = { filed: B.green, ready: B.primary, pending: B.light, overdue: B.red, "—": "#E2E8F0" };
...
const Badge = ({ status }) => {
  const m = { overdue:{bg:B.redBg,c:B.redText,b:"#FECACA",l:"Overdue"}, "due-soon":{bg:B.amberBg,c:B.amberText,b:"#FDE68A",l:"Due soon"}, filed:{bg:B.greenBg,c:B.greenText,b:"#A7F3D0",l:"Filed"}, "pending-invite":{bg:B.purpleBg,c:B.purpleText,b:"#DDD6FE",l:"Pending"} };
    ...
    <div style={{ display:"flex", height:"100vh", fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif", background:B.surface, color:B.text }}>
      ...
      <div style={{ width:230, background:B.navy, display:"flex", flexDirection:"column", flexShrink:0 }}>
        ...
        <div style={{ padding:"16px 12px", flex:1 }}>
          ...
          <NavItem label="Chase manager" icon="↗" count={overdueCount} />
          ...
          <NavItem label="Add client" icon="+" />
          <NavItem label="Settings" icon="⚙" />
          <NavItem label="HMRC connection" icon="⟷" />
        </div>

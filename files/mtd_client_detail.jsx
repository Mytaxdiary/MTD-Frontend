                        ...
                        <div style={{ flex:1, paddingBottom:20, paddingLeft:12 }}>
                          ...
                          {q.status === "filed" && (
                            <div style={{ display:"flex", gap:24, marginTop:8, fontSize:12, color:B.muted }}>
                              ...
                              <span>Income: <b style={{ color:B.text }}>£{q.income.toLocaleString()}</b></span>
                              <span>Expenses: <b style={{ color:B.text }}>£{q.expenses.toLocaleString()}</b></span>
                              <span>Net: <b style={{ color:B.text }}>£{(q.income-q.expenses).toLocaleString()}</b></span>
                            </div>
                          )}
                          {q.status === "pending" && (
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12, marginBottom:16 }}>
                      {filedQs.map(q=>(
                        <div key={q.q} style={{ padding:"12px", background:B.surface, borderRadius:8, border:`1px solid ${B.borderLight}` }}>
                          ...
                          <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:3 }}><span style={{ color:B.muted }}>Income</span><span style={{ fontWeight:600, fontVariantNumeric:"tabular-nums" }}>£{q.income.toLocaleString()}</span></div>
                          <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:3 }}><span style={{ color:B.muted }}>Expenses</span><span style={{ fontWeight:600, fontVariantNumeric:"tabular-nums" }}>£{q.expenses.toLocaleString()}</span></div>
                          <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, borderTop:`1px solid ${B.borderLight}`, paddingTop:4 }}><span style={{ fontWeight:600 }}>Net</span><span style={{ fontWeight:700, fontVariantNumeric:"tabular-nums", color:B.greenText }}>£{(q.income-q.expenses).toLocaleString()}</span></div>
                        </div>
                      ))}
                    </div>
